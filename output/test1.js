...
//   standalone : true,
//   imports : [CommonModule],
//   template: `<div *ngIf="product; else noProduct" class="product-detail"><img [src]="product.image" alt="{{ product.name }}" /><h2>{{ product.name }}</h2><p>{{ product.details }}</p><button (click) = "editProduct()" > Edit></button><button (click) = "deleteProduct(product.id)" > delete></button><button (click) = "goToList()" > Back></button></div><ng-template></ng-template>`,
//   styles: [`
//   .product-detail {
//   padding : 20
//   px;
